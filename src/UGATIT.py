...
class UGATIT(object):
    ...
    def test_endpoint(self, input_path, output_path):
        print(input_path)
        print(output_path)
        my_img = np.asarray(self.load_img(input_path))
        fake_img = self.sess.run(self.test_fake_B, feed_dict={self.test_domain_A: my_img})
        save_images(fake_img, [1, 1], output_path)
        return output_path

    def load_img(self, image_path, size=256):
        print(image_path)
